function main(config) {
  // --- 1. 定义 rule-providers ---
  // 如果原配置中没有 rule-providers，先初始化为空对象
  ...
  // 将 providers 合并到 config['rule-providers'] 中
  Object.assign(config['rule-providers'], providers);

  // --- 2. 覆写 GeoIP 数据库 (可选) ---
  // 如果原配置中没有 geodata，先初始化为空对象
  if (!config.geodata) {
    config.geodata = {};
  }
  // 设置 geoip 地址
  config.geodata.geoip = 'https://raw.githubusercontent.com/Loyalsoldier/geoip/release/Country.mmdb';

  // --- 3. 覆写规则 (白名单模式) ---
  // 确保 config.rules 是一个数组
